                     ...
                     bool init_random)
{
  if (useRef)
  {
    auto* spo_main = new miniqmcreference::einspline_spo_ref<D, OHMMS_PRECISION>;
  ...
  {
    auto* spo_main = new einspline_spoD, OHMMS_PRECISION>;
    spo_main->set(nx, ny, nz, num_splines, nblocks);
    spo_main->Lattice.set(lattice_b);
    return dynamic_cast<SPOSet*>(spo_main);
  }
...
SPOSet* build_SPOSet_view(bool useRef, const SPOSet* SPOSet_main, int team_size, int member_id)
{
  ...
  {
    auto* temp_ptr =
      dynamic_cast<const miniqmcreference::einspline_spo_ref<D,OHMMS_PRECISION>*>(SPOSet_main);
    auto* spo_view =
      new miniqmcreference::einspline_spo_ref<D,OHMMS_PRECISION>(*temp_ptr, team_size, member_id);
    return dynamic_cast<SPOSet<D>*>(spo_view);
  }
  else
  {
